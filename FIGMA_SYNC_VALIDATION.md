...
## Code caught up to Figma — 2026-08-27

The fourth pass above left Figma ahead of code on purpose (`primitive/font/size/sm`/`primitive/font/lineHeight/sm` existed only in Figma). Andreas asked directly whether Figma/code/Storybook were actually in sync — they weren't — and chose to close the gap rather than leave it open.

- Added `font.size.sm` (12px) / `font.lineHeight.sm` (16px) to `tokens.json` (with `useFor`/`avoidFor`, same convention as the other semantic-adjacent tokens), ran `npm run tokens:build` — `--font-size-sm`/`--font-line-height-sm` now exist in `src/styles/tokens.css`.
- `form-field.tsx`'s error `<p>` now always renders (previously conditional on `error`), using `invisible` + `aria-hidden` when there's no error instead of not rendering at all — same "reserve the height, toggle visibility" pattern already built into Figma's `Input` component set. Also switched from `--font-size-md`/`--font-line-height-md` to the new `sm` pair, matching Figma's `Caption` style.
- Updated `form-field.meta.json` (new tokens listed, `figmaNote` corrected — the error-row pattern is now mirrored in Figma even though `FormField` itself still has no dedicated node) and `form-field.guidelines.md` (documents the always-rendered row explicitly, since it's a real behavior change any future caller needs to know about).
- **Verified in the browser, not just build-green:** `npm run build`/`lint` clean; Storybook's `FormField — Default` vs `— With Error` compared directly via DOM measurement (`getBoundingClientRect`) — the error row is `16px` tall either way (`visibility: hidden` in Default, visible in Error), so the input's vertical position never shifts between states. `LoginPage`/`SignUpPage` stories screenshotted — the reserved gap under each field doesn't read as awkward or broken in a real composed form. `npm run test:a11y`: all 18 stories green, no violations.
- Removed the two `sm` variables from `MANUALLY_MAINTAINED_VARIABLES` in `sync-figma-variables.js` — they're no longer Figma-only, so they belong on the normal primitive sync path like every other token, not the manual-exception list. Confirmed the generated `tokens.figma.json` names (`primitive/font/size/sm`, `primitive/font/lineHeight/sm`) match what was hand-created in Figma exactly, so the next `npm run tokens:sync:figma` run will update the existing variables in place, not create duplicates.
