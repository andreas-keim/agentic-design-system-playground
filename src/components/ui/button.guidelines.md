...
- **outline** / **secondary**: sekundäre Aktion neben einer `primary`-Aktion (z. B. "Abbrechen" neben "Speichern"). Beide aktuell visuell sehr ähnlich (kein eigenes Token unterscheidet sie) — Wahl zwischen beiden ist noch keine bewusste Design-Entscheidung, sondern shadcn-Default. **Interims-Regel, bis eine echte Abgrenzung entschieden ist:** neuen Code auf `outline` festlegen, `secondary` nicht weiter neu einsetzen — verhindert, dass zwei optisch identische, aber technisch verschiedene Pfade unkontrolliert nebeneinander wachsen. Vormerken für eine spätere Abgrenzung.
...
- **link**: für Aktionen, die sich wie Inline-Text-Navigation verhalten sollen, nicht wie ein "echter" Button-Look (kein Fill/Rand). **Bekannte Lücke (gefunden durch zwei unabhängige Agenten-Testläufe, Step 7):** `size` ist eine von `variant` unabhängige `cva`-Achse — `link` erbt bei `size="default"` trotzdem das volle Padding/die volle Höhe der Default-Size, sieht also ohne Weiteres nicht wie Inline-Text aus. Beide Testläufe haben das unabhängig voneinander mit `className="h-auto p-0"` gefixt. Das ist der dokumentierte, richtige Weg (kein Eingriff in `button.tsx` nötig) — bis eine eigene, kleinere Size für Inline-Links entschieden wird, immer `h-auto p-0` bei `variant="link"` setzen.
